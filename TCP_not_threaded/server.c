...
struct HTTPHeader* parseHTTP(char *buffer)
{
	...
	header->filename++;//to eliminate '/' in front of filename
...
void handle_client(int *clienfd)
{
	char buff[256];
	int n;
...
{
	int servfd, clienfd, portno, n, i, fd_buff[NTHREADS],val = 1;
	socklen_t clilen;
